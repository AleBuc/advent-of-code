...
/*        String schema = "";*/
        ...
        List<Integer> numbersToSum = numbers.stream().filter(number -> !number.symbols.isEmpty()).map(Number::getNumber).toList();
        System.out.println(numbersToSum);
        int sum = numbersToSum.stream().reduce(0, Integer::sum);
