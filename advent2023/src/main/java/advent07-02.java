...
class Advent0702 {
    ...
    static Type determinateType(List<Card> cards) {
        ...
        int jokerCount = countByCard.getOrDefault(Card.JOKER, 0);
        if (countByCard.containsValue(5) ||
            (jokerCount != 0 && countByCard.containsValue(5 - jokerCount))) {
            return Type.FIVE_OF_A_KIND;
        }
        if (countByCard.containsValue(4) ||
            (jokerCount != 0 && countByCard.containsValue(4 - jokerCount))) {
            return Type.FOUR_OF_A_KIND;
        }
        if ((countByCard.containsValue(3) && countByCard.containsValue(2))
            || (jokerCount == 1 && (
                countByCard.entrySet().stream().filter(entry -> entry.getValue().equals(2)).count() == 2
                || countByCard.containsValue(3))
            || (jokerCount == 2 && countByCard.entrySet().stream().filter(entry -> entry.getValue().equals(2)).count() == 2 && countByCard.containsValue(1)))) {
            return Type.FULL_HOUSE;
        }
        if (countByCard.containsValue(3) ||
            (jokerCount != 0 && countByCard.containsValue(3 - jokerCount))) {
            return Type.THREE_OF_A_KIND;
        }
        if ((countByCard.entrySet().stream().filter(entry -> entry.getValue().equals(2)).count() == 2)
            || jokerCount == 1 && countByCard.entrySet().stream().filter(entry -> entry.getValue().equals(2)).count() == 1) {
