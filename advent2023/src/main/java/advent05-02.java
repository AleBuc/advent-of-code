...
/*        String puzzle = "";*/
    ...
    static HashMap<SeedBag, SeedBag> transformValuesFromSourceToDestination(HashMap<SeedBag, SeedBag> seedsBySource, List<Almanac.Entry> almanacEntries) {
        ...
        HashMap<SeedBag, SeedBag> toProcess = new HashMap<>(seedsBySource);

        int renew;

        do {
            renew=0;
            List<Map.Entry<SeedBag, SeedBag>> seedEntries = toProcess.entrySet().stream().toList();
            HashMap<SeedBag, SeedBag> toReProcess = new HashMap<>();

            for (Map.Entry<SeedBag, SeedBag> seedEntry : seedEntries) {

                for (Almanac.Entry almanacEntry : almanacEntries) {
                    //if contains in range
                    if (seedEntry.getValue().min >= almanacEntry.getSource() && seedEntry.getValue().max <= almanacEntry.getSourceMax()) {
                        ...
                        toProcess.remove(seedEntry.getKey());
                        ...
                        SeedBag seedBag1Value = new SeedBag(seedEntry.getValue().min, almanacEntry.getSourceMax());
                        SeedBag seedBag1Key = new SeedBag(seedEntry.getKey().min, seedEntry.getKey().min + diff1);
                        ...
                        SeedBag seedBag3Value = new SeedBag(seedEntry.getValue().min + diff1 + 1, seedEntry.getValue().max - diff2 - 1);
            ...
            if (!toProcess.isEmpty()){
                result.putAll(toProcess);
            }
            toProcess = toReProcess;
